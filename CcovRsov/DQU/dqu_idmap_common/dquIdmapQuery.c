...
*返回值：1成功，失败
*改动记录：优化算法，查询时不再遍历所有轨道区段信息，直接依据LinkID信息获取对应轨道区段结构体 m by sds 2019-6-26
******************************************************/
UINT8 dquQueryTrackIdOffset(UINT16 linkId, UINT32 linkOffset, UINT32 *pTrackSecId, UINT32 *pTrackOffset)
{
	...
	UINT16 trackStepIndex=0;/*用于获取轨道区段数据在指针索引中的偏移信息*/
	UINT8 failCode=0;

	if (DQU_IDMAP_INIT_TRUE==qds_IsInitIdMapStruPtr)
	{
		/*判定LinkID的有效范围*/
		if ((0 != linkId)&&(0xFFFF != linkId) && (linkId <= qds_dsuIdmapIndexStru->TRACKSECNUMINDEXSIZE))
		{
			/*获取link偏移索引*/
			trackStepIndex=qds_dsuIdmapIndexStru->dsuTrackSecNumIndex[linkId];

			/*查询到的索引信息需为有效值*/
			if (0xFFFF != trackStepIndex)
			{
				/*获取到link对应的轨道区段结构体指针位置*/
				pTrackSecStru=qds_dsuStaticIdmapStru->pTrackSecNumStru+trackStepIndex;

			/*轨道区段结构体指针不为空，进行计算偏移量处理*/
			if ((NULL != pTrackSecStru) && (linkId == pTrackSecStru->wLinkId))
			{
				/*遍历Link对应的轨道区段信息 查询偏移量信息*/
