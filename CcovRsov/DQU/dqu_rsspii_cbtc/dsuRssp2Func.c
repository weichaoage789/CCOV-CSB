...
UINT8 dsuRssp2Init(UINT8* FSName)
{
	...
	DSU_IDMAP_STRU pDsuIdmapStru; /*编号对照表*/

	/*初始化编号对照表*/
	dquSetInitIdmapRef(&pDsuIdmapStru);
	dfsIdmapInit(FSName);
...
UINT8 dquGetEtcsIdByIpPort(IN DSU_IP_PORT IpPort, OUT UINT32 *pEtcsId)
{
	...
	if ((NULL != pEtcsId) && (NULL != g_dsuRssp2Stru))
	{
		*pEtcsId = 0;
		...
		for (i = 0; i < g_dsuRssp2Stru->dsuRssp2DevCfgLen; i++)
		{
			rslt1 = CommonMemCmp(IpPort.Ip, pDevCfg->RedIpPort.Ip, 4);
			rslt2 = CommonMemCmp(IpPort.Ip, pDevCfg->BlueIpPort.Ip, 4);
...
*函数名:	CommonMemCmp
*功能描述:	公共内存比较函数
*输入参数:	const void *str1	比较1数据
*			const void *str2	比较2数据
*			UINT32 count	数据长度
*输入出参:	无
*输出参数:	无
*返回值:	INT32 < 0:小于;>0:大于;=0相等
*/
INT32 CommonMemCmp(const void *str1, const void *str2, UINT32 count)
{
	INT32 rtn = 0;	/*返回值*/
	UINT32 i = 0;	/*用于循环*/
	...
	{
		if (((UINT8 *)str1)[i] != ((UINT8 *)str2)[i])	/*不等,退出循环,返回差值*/
		{
			rtn = ((UINT8 *)str1)[i] - ((UINT8 *)str2)[i];
			break;
		}
	}
