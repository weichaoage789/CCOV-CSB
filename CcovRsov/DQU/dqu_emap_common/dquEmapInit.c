...
static UINT32	g_FilterStopPointAttr = 0x0;  /*需要过滤停车点时，使用的停车点属性过滤条件，该值由上层应用输入 add by sds 2019-10-22*/
...
{
	{DQU_QFUN_TRUE, EMAP_NOP_0,					initNop,			5,1,0},	/*为使数据表序号与该数据表在数组中所占位置相同，补全数据 ID  m by sds 2019-10-22*/		/*0*/
	{DQU_QFUN_TRUE, EMAP_LINK_ID,				initLink,			5,1,0},	/*link数据ID*/							/*01*/				
	{DQU_QFUN_TRUE, EMAP_POINT_ID,				initPoint,			5,1,0},	/*道岔数据ID*/							/*02*/	
	{DQU_QFUN_TRUE, EMAP_SIGNAL_ID,				initSignal,			5,1,0},	/*信号机数据ID*/						/*03*/	
	{DQU_QFUN_TRUE, EMAP_BALISE_ID,				initBalise,			5,1,0},	/*应答器数据ID*/						/*04*/	
	{DQU_QFUN_TRUE, EMAP_AXLE_ID,				initAxle,			5,1,0},	/*记轴器数据ID*/						/*05*/	
	{DQU_QFUN_TRUE, EMAP_SCREEN_ID,				initScreen,			5,1,0},	/*安全屏蔽门数据ID*/					/*06*/	
	{DQU_QFUN_TRUE, EMAP_EMERG_STOP_BUTTON_ID,	initEmergStop,		5,1,0},	/*紧急停车按钮数据ID*/					/*07*/	
	{DQU_QFUN_TRUE, EMAP_STATION_ID,			initStation,		5,1,0},	/*停车区域数据ID*/						/*08*/	
	{DQU_QFUN_TRUE, EMAP_STOPPOINT_ID,			initStopPoint,		5,1,0},	/*停车点数据ID*/						/*09*/	
	...
	{DQU_QFUN_TRUE, EMAP_NOP_42,				initNop,			5,1,0},	/*补全数据 ID sds */							/*42*/
	{DQU_QFUN_TRUE, EMAP_NOP_43,				initNop,			5,1,0},	/*补全数据 ID sds */							/*43*/
	{DQU_QFUN_TRUE, EMAP_NOP_44,				initNop,			5,1,0},	/*补全数据 ID sds */							/*44*/
	{DQU_QFUN_TRUE, EMAP_NOP_45,				initNop,			5,1,0},	/*补全数据 ID sds */							/*45*/
	{DQU_QFUN_TRUE, EMAP_NOP_46,				initNop,			5,1,0},	/*补全数据 ID sds */							/*46*/
	...
	{DQU_QFUN_TRUE, EMAP_ADJTSTATION_ID,		initAdjtStation,	5,1,0},	/*依据互联互通数据结构V18.0.0添加，add by sds 20200211*/							/*52*/
	{DQU_QFUN_TRUE, EMAP_PSC_ID,				initPSC,			5,1,0},	/*补全数据 ID sds */							/*53*/
	...
	{DQU_QFUN_TRUE, EMAP_NOP_61,				initNop,			5,1,0},	/*补全数据 ID sds */							/*61*/
	{DQU_QFUN_TRUE, EMAP_NOP_62,				initNop,			5,1,0},	/*补全数据 ID sds */							/*62*/
	{DQU_QFUN_TRUE, EMAP_NOP_63,				initNop,			5,1,0},	/*补全数据 ID sds */							/*63*/
	{DQU_QFUN_TRUE, EMAP_NOP_64,				initNop,			5,1,0},	/*补全数据 ID sds */							/*64*/
	{DQU_QFUN_TRUE, EMAP_NOP_65,				initNop,			5,1,0},	/*补全数据 ID sds */							/*65*/
	{DQU_QFUN_TRUE, EMAP_NOP_66,				initNop,			5,1,0},	/*补全数据 ID sds */							/*66*/
	{DQU_QFUN_TRUE, EMAP_NOP_67,				initNop,			5,1,0},	/*补全数据 ID sds */							/*67*/
	{DQU_QFUN_TRUE, EMAP_NOP_68,				initNop,			5,1,0},	/*补全数据 ID sds */							/*68*/
	{DQU_QFUN_TRUE, EMAP_NOP_69,				initNop,			5,1,0},	/*补全数据 ID sds */							/*69*/
	{DQU_QFUN_TRUE, EMAP_NOP_70,				initNop,			5,1,0},	/*补全数据 ID sds */							/*70*/ 
	{DQU_QFUN_TRUE, EMAP_NOP_71,				initNop,			5,1,0},	/*补全数据 ID sds */							/*71*/
	{DQU_QFUN_TRUE, EMAP_NOP_72,				initNop,			5,1,0},	/*补全数据 ID sds */							/*72*/
	{DQU_QFUN_TRUE, EMAP_NOP_73,				initNop,			5,1,0},	/*补全数据 ID sds */							/*73*/
	{DQU_QFUN_TRUE, EMAP_NOP_74,				initNop,			5,1,0},	/*补全数据 ID sds */							/*74*/
	{DQU_QFUN_TRUE, EMAP_NOP_75,				initNop,			5,1,0},	/*补全数据 ID sds */							/*75*/
	{DQU_QFUN_TRUE, EMAP_NOP_76,				initNop,			5,1,0},	/*补全数据 ID sds */							/*76*/
	{DQU_QFUN_TRUE, EMAP_NOP_77,				initNop,			5,1,0},	/*补全数据 ID sds */							/*77*/
	{DQU_QFUN_TRUE, EMAP_NOP_78,				initNop,			5,1,0},	/*补全数据 ID sds */							/*78*/
	{DQU_QFUN_TRUE, EMAP_NOP_79,				initNop,			5,1,0},	/*补全数据 ID sds */							/*79*/
	{DQU_QFUN_TRUE, EMAP_NOP_80,				initNop,			5,1,0}	/*补全数据 ID sds */							/*80*/ 
};
/*初始化任务登记表项目数量*/
...
/*根据版本配置数据处理过程函数
**修改记录：
**1、依据互联互通数据结构V18.0.0修改，m by sds 20200211
**2、修改不需要初始化的表格信息项 m by sds 2020-8-19
*/
static UINT8 initConfigEmapTaskTab()
{
	...
	/*为实现数据表与实际表数值一一对应，所以从0开始 补全数据 ID 因为是空表，所以设置不进行读取 add by sds 2019-10-22*/
	g_struInitTaskTab[EMAP_NOP_0].bIsEffective	= DQU_QFUN_FASLE;/*为补全数据表占位的空表0*/

	/* 不需要处理的空数据表 */
	g_struInitTaskTab[EMAP_NOP_40].bIsEffective	= DQU_QFUN_FASLE;/*为补全数据表占位的空表40*/
	for(loopIndex = EMAP_NOP_42; loopIndex <= EMAP_NOP_50; loopIndex++)
	{
		g_struInitTaskTab[loopIndex].bIsEffective = DQU_QFUN_FASLE;/*为补全数据表占位的空表42~50*/	
	}
	for(loopIndex = EMAP_NOP_54; loopIndex <= EMAP_NOP_80; loopIndex++)
	{
		g_struInitTaskTab[loopIndex].bIsEffective = DQU_QFUN_FASLE;/*为补全数据表占位的空表54~80*/	
	}
	...
	if (DQU_CBTC_CPK == cbtcVersionType)/*以前版本合库不需要处理的数据表*/
	{
		...
		g_struInitTaskTab[EMAP_ADJTSTATION_ID].bIsEffective		= DQU_QFUN_FASLE;/*临线停车区域ID*/
		g_struInitTaskTab[EMAP_PSC_ID].bIsEffective		        = DQU_QFUN_FASLE;/*PSC表*/

		/*g_struCalcTaskTab[MAX_CALCTASK_COUNT-2].bIsEffective=DQU_QFUN_FASLE;*//*不需要建立Link和其他设备间的映射关系*/
	...
	else if (DQU_CBTC_DG == cbtcVersionType)/*单轨不需要处理的数据表*/
	{
		...
		/*g_struCalcTaskTab[MAX_CALCTASK_COUNT-1].bIsEffective	= DQU_QFUN_FASLE;*//*不需要建立Link和其他设备间的映射关系*/
		g_struCalcTaskTab[MAX_CALCTASK_COUNT-2].bIsEffective	= DQU_QFUN_FASLE;/*不需要建立Link与坡度索引间的映射关系*/
		g_struCalcTaskTab[MAX_CALCTASK_COUNT-1].bIsEffective	= DQU_QFUN_FASLE;/*不需要计算SPKS包含的Link序列*/
...
/************************************************************
*函数功能：设置过滤停车点属性的过滤信息，如果不设置则不进行过滤
*参数说明：
*		filterAttr:过滤属性信息
*返回值：无
*修改记录：add by sds 2019-10-22
*************************************************************/
void SetFilterStopPointAttr(UINT32 filterAttr)
{
	g_FilterStopPointAttr = filterAttr;
}
...
UINT8 ARInStru()
{
	...
	UINT16 arInStopPointIndex = DSU_NULL_16;                 /*无人折返进入停车点索引*/
	UINT16 revertStopPointIndex = DSU_NULL_16;               /*无人折返换端停车点索引*/
	...
	if (0 != dsuDataLenStru->wARLen)
	{
		...
		if (DQU_QFUN_TRUE==EmapCalcFunStepCtrl(0x02))
		{
			for (wIndex = g_struCalcTaskTab[g_wCurCalcStruProg].wCurInitPos; wIndex < dsuDataLenStru->wARInLen; wIndex++)
			{
				...
				/*获取无人折返进入和换端停车点的数据指针索引*/
				arInStopPointIndex = dsuStopPointIndex[pTempARStru->wARInStopPointId];
			    revertStopPointIndex = dsuStopPointIndex[pTempARStru->wRevertPointId];

				/*因为实现了停车点过滤功能，因此此处需要防护，可能有的停车点无法查到有效的数据指针  m by sds  2019-10-22*/
				if ((DSU_NULL_16 == arInStopPointIndex) || (DSU_NULL_16 == revertStopPointIndex))
				{
					pTempARInStru->wStopPointLinkID = 0;
					pTempARInStru->dwStopPointLinkOffset = 0;
					pTempARInStru->wDir = 0;
					pTempARInStru->dwParkArea = 0;
					pTempARInStru->wRevertPointLinkID = 0;
					pTempARInStru->dwRevertPointLinkOffset = 0;
					pTempARInStru->dwRevertParkArea = 0;
					pTempARInStru->wARLampID = 0;
					pTempARInStru->wARButtonID = 0;
				} 
				else
				{
...
{	
	...
	UINT16 arOutStopPointIndex = DSU_NULL_16;                 /*无人折返退出停车点索引*/
	...
	if (0 != dsuDataLenStru->wARLen)
	{
		...
		if (DQU_QFUN_TRUE==EmapCalcFunStepCtrl(0x02))
		{
			for (wIndex = g_struCalcTaskTab[g_wCurCalcStruProg].wCurInitPos; wIndex < dsuDataLenStru->wAROutLen; wIndex++)
			{
				...
				/*获取无人折返退出停车点的数据指针索引*/
				arOutStopPointIndex = dsuStopPointIndex[pTempARStru->wAROutStopPointId];

				/*因为实现了停车点过滤功能，因此此处需要防护，可能有的停车点无法查到有效的数据指针  m by sds  2019-10-22*/
				if (DSU_NULL_16 == arOutStopPointIndex)
				{
					pTempAROutStru->wStopPointLinkID = 0;
					pTempAROutStru->dwStopPointLinkOffset = 0;
					pTempAROutStru->dwParkArea = 0;
...
备注：内部调用函数，互联互通使用
修改记录:
1、更新进路范围内CBTC接近区段的范围判断 使用宏值信息替代，防止遗忘更新,不修复编码规范问题 m by sds 2020-8-21
*********************************************/
	...
	***********/
	if (DQU_QFUN_TRUE==EmapCalcFunStepCtrl(0x02))
	{
		for (i = g_struCalcTaskTab[g_wCurCalcStruProg].wCurInitPos; i < dsuDataLenStru->wRouteLen; i++)
		{
			...
			/*如果CBTC接近区段>MAX_CBTC_ACCESS_LINK_NUM，数据出错，退出*/
			...
			/*如果CBTC接近区段为大于0，小于10*/
			else if (0 < pRouteStru->wCBTCAccessLinkNum)
			{
				for (k=0;k<pRouteStru->wCBTCAccessLinkNum;k++)
				{
					/*填充数据*/
...
/**********************************************
函数功能：初始化[占位表格数组]:只用作在任务数组中占位，保持数据表单号与任务数据中索引一致
...
返回值:2处理中,1成功,0失败
修改记录：
1、清空实际行数计数变量，防止跨线等场景下重复调用本函数，导致实际函数信息错误 m by sds 2020-5-21
**********************************************/
UINT8 initStopPoint(UINT8 *pDataAddr, UINT32 dataSize, UINT16 rowCount)
{
	...
	UINT16 j=0;
	static UINT32 rowDataLen=0;
    static UINT32 realLoadRowLen = 0;       /*实际加载的数据行数*/
	UINT8 ftnValue = 0;                     /*局部函数返回值存储变量*/
	UINT16 wStruDataLen=0;
	UINT16 wDataIndexLen=0;
	...
	{		
		...
		else
		{
			...
			/*索引数组长度计算，行数输入的属性输入0，为了让函数内的过滤条件=TRUE*/
			STOPPOINT_STRU_init(pDataAddr + dataSize - rowDataLen, &stru, &realLoadRowLen ,0);
			
			/*此步骤是为了计算最大索引ID，但是也会把realLoadRowLen++，但此时++会导致实际行数错误，所以减回来*/
			realLoadRowLen = (1 <= realLoadRowLen) ? (realLoadRowLen - 1) : (realLoadRowLen);
	...
	if (2 == rtnValue && DQU_QFUN_TRUE == EmapInitFunStepCtrl(0x02))
	{
		...
		{
			/*过滤属性为默认值时才判断数据范围有效性，否则在第3阶段过滤后判断*/
			if (0x0 == g_FilterStopPointAttr)
			{
				/*检测电子地图对应数据项数组的个数是否超出内存分配范围*/
	...
	if (2 == rtnValue && DQU_QFUN_TRUE == EmapInitFunStepCtrl(0x03))
	{
		...
		{
			/*需要过滤属性，没次处理都有可能导致数组越界，需要每次进入时均排查逻辑*/
			if (0x0 != g_FilterStopPointAttr)
			{
				/*检测电子地图对应数据项数组的个数是否超出内存分配范围*/
				dquGetEmapDataStruArrayLen(EMAP_STOPPOINT_ID, &wStruDataLen);

				if (((0 != wStruDataLen) && (realLoadRowLen >= wStruDataLen)) || (0 == wStruDataLen))
				{
					rtnValue=0;
					break;
				}
			...
			pCurrentStru = dsuStaticHeadStru->pStopPointStru + realLoadRowLen;

			pData = pDataAddr + rowDataLen*j;

			/*填充数据结构体*/
			ftnValue = STOPPOINT_STRU_init(pData, pCurrentStru,&realLoadRowLen,g_FilterStopPointAttr);

			/*解析成功，满足过滤条件*/
			if (1 == ftnValue)
			{
				/*完成对应填充索引*/
				dsuEmapIndexStru->dsuStopPointIndex[pCurrentStru->wId] = (1 <= realLoadRowLen) ? (realLoadRowLen - 1) : (realLoadRowLen);
			}
		...
		{
			/*将实际加载的数据行数赋予给指针内的行数存储字段*/
			dsuDataLenStru->wStopPointLen = realLoadRowLen;

			/*清空实际行数计数变量，防止跨线等场景下重复调用本函数，导致实际函数信息错误*/
			realLoadRowLen = 0;
...
UINT8 initStaticResSpeed(UINT8 *pDataAddr, UINT32 dataSize, UINT16 rowCount)
{
	...
	else if ((DQU_CBTC_CPK == cbtcVersionType) || (DQU_CBTC_HLT == cbtcVersionType)||(DQU_CBTC_DG == cbtcVersionType) )/*根据需求互联互通与合库处理一致*/
	{
...
UINT8 initPlatForm(UINT8 *pDataAddr, UINT32 dataSize, UINT16 rowCount)
{
	...
	UINT16 wStruDataLen=0;
	UINT16 wDataIndexLen=0;
	static UINT16 logicSgmtSumCount = 0;/*站台对应的触发区段ID数量*/
	...
	{		
		...
		else
		{
			...
			/*站台对应的触发逻辑区段ID个数=(行字节数-非触发区段ID信息字节数)/单个触发区段ID字节数*/
			logicSgmtSumCount = ((UINT16)rowDataLen - STATION_NO_LOGICSGMT_SIZE) / 2;

			/*动态计算出的字段数量大于数组最大存储量，报数据错误*/
			if (STATION_INCLUDE_LOGICSGMT_NUM < logicSgmtSumCount)
			{
				rtnValue = 0;
			} 
			else
			{
				/*索引数组长度计算*/
				DSU_PLAT_FORM_STRU_init(pDataAddr + dataSize - rowDataLen, &stru,logicSgmtSumCount);
	...
	if (2 == rtnValue && DQU_QFUN_TRUE == EmapInitFunStepCtrl(0x03))
	{
		...
		for (j = g_struInitTaskTab[g_wCurInitStruProg].wCurInitPos; j < dsuDataLenStru->wPlatFormLen; j++)
		{
			...
			pData = pDataAddr + rowDataLen*j;
			/*填充数据结构体*/
			DSU_PLAT_FORM_STRU_init(pData, pCurrentStru,logicSgmtSumCount);
...
UINT8 initAdjtStation(UINT8 *pDataAddr, UINT32 dataSize, UINT16 rowCount)
{
	...
	UINT8 *pData = NULL;
	DSU_AdjtStation_STRU stru ={0};					/*临线停车区结构体临时变量*/
	DSU_AdjtStation_STRU *pCurrentStru = NULL;	/*临线停车区结构体临时指针变量*/
	...
	if (2 == rtnValue && DQU_QFUN_TRUE == EmapInitFunStepCtrl(0x01))
	{		
		dsuDataLenStru->wAdjtStationLen = rowCount;
		...
		else
		{
			...
			/*索引数组长度计算*/
			ADJTSTATION_STRU_init(pDataAddr + dataSize - rowDataLen, &stru);

			/*索引个数为最大ID + 1*/
			dsuEmapIndexStru->ADJTSTATIONINDEXNUM = stru.wId;			
		}		
	}
	...
	if (2 == rtnValue && DQU_QFUN_TRUE == EmapInitFunStepCtrl(0x02))
	{
		...
		{
			/*动态申请索引空间*/
			dsuEmapIndexStru->dsuAdjtStationIndex = (UINT16*)MyNew(sizeof(UINT16) * dsuEmapIndexStru->ADJTSTATIONINDEXNUM + 2);

			/*动态申请空间失败*/
			if (NULL == dsuEmapIndexStru->dsuAdjtStationIndex)
			{
				rtnValue = 0;
			}
			else
			{
				/*初始化为0xFF*/
				memset(dsuEmapIndexStru->dsuAdjtStationIndex, 0xFF, dsuEmapIndexStru->ADJTSTATIONINDEXNUM * 2 + 2);
			}

			if (2 == rtnValue)
			{
				/*动态申请空间*/
				dsuStaticHeadStru->pAdjtStationStru = (DSU_AdjtStation_STRU *)MyNew(sizeof(DSU_AdjtStation_STRU) * dsuDataLenStru->wAdjtStationLen);

				/*动态申请空间失败*/
				if (NULL == dsuStaticHeadStru->pAdjtStationStru)
				{
					rtnValue = 0;
				}
				else
				{
					/*内存先设为无效*/
					memset(dsuStaticHeadStru->pAdjtStationStru, 0xFF, sizeof(DSU_AdjtStation_STRU) * dsuDataLenStru->wAdjtStationLen);
				}
			}
		}
		else/*内存空间是由外部产品来分配的*/
		{
			/*检测电子地图对应数据项数组的个数是否超出内存分配范围*/
			dquGetEmapDataStruArrayLen(EMAP_ADJTSTATION_ID, &wStruDataLen);

			if (((0 != wStruDataLen) && (dsuDataLenStru->wAdjtStationLen >= wStruDataLen))||(0 == wStruDataLen))
			{
				rtnValue=0;
			}
			else
			{
				/*检测电子地图中对应索引数组的个数是否超出内存分配范围*/
				dquGetEmapDataIndexArrayLen(EMAP_ADJTSTATION_ID, &wDataIndexLen);
				if (((0!=wDataIndexLen) && (dsuEmapIndexStru->ADJTSTATIONINDEXNUM >= wDataIndexLen))||(0 == wDataIndexLen))
				{
					rtnValue=0;
				}
			}
	...
	{
		/*解析数据到 自动折返 结构体数组 */
		for (j = g_struInitTaskTab[g_wCurInitStruProg].wCurInitPos; j < dsuDataLenStru->wAdjtStationLen; j++)
		{
			pCurrentStru = dsuStaticHeadStru->pAdjtStationStru + j;

			pData = pDataAddr + (rowDataLen * j);

			/*填充数据结构体*/
			ADJTSTATION_STRU_init(pData, pCurrentStru);

			/*完成对应填充索引*/
			dsuEmapIndexStru->dsuAdjtStationIndex[pCurrentStru->wId] = j;
		...
		if (j == dsuDataLenStru->wAdjtStationLen)
		{
			rtnValue = 1;
		}
	}
...
* 返回值:		2未处理完，1成功，0失败
8 修改记录:
* 1、依据互联互通数据结构V20.0.0添加，add by sds 2020-8-18
**************************************************************************/
UINT8 initPSC(UINT8 *pDataAddr, UINT32 dataSize, UINT16 rowCount)
{
	...
	UINT8 rtnValue = 2;	                     /*返回值变量声明*/
	UINT8 *pData = NULL;
	DSU_PSC_STRU stru ={0};					 /*PSC结构体临时变量*/
	DSU_PSC_STRU *pCurrentStru = NULL;	     /*PSC结构体临时指针变量*/
	UINT16 j = 0;
	static UINT32 rowDataLen=0;
	UINT16 wStruDataLen = 0;
	UINT16 wDataIndexLen = 0;
	UINT32 funErrCode = DSU_FUNC_SUCESS;
	...
	if(NULL != pDataAddr)
	{
		...
		if (NULL != g_dsuEmapStru)
		{
			...
			{		
				...
				dsuDataLenStru->wPSCLen = rowCount;
				...
				else
				{
					...
					/*索引数组长度计算*/
					PSC_STRU_init((pDataAddr + dataSize - rowDataLen), &stru);

					/*索引个数为最大ID + 1*/
					dsuEmapIndexStru->PSCINDEXNUM = stru.wId;			
				}		
			}
			else
			{
			...
			if ((2 == rtnValue) && (DQU_QFUN_TRUE == EmapInitFunStepCtrl(0x02)))
			{
				...
				{
					/*动态申请索引空间*/
					dsuEmapIndexStru->dsuPSCIndex = (UINT16*)MyNew(sizeof(UINT16) * dsuEmapIndexStru->PSCINDEXNUM + 2);

					/*动态申请空间失败*/
					if (NULL == dsuEmapIndexStru->dsuPSCIndex)
					{
						rtnValue = 0;
						funErrCode = DSU_FUNC_CODE3;
					}
					else
					{
						/*初始化为0xFF*/
						memset(dsuEmapIndexStru->dsuPSCIndex, 0xFF, dsuEmapIndexStru->PSCINDEXNUM * 2 + 2);

						/*动态申请空间*/
						dsuStaticHeadStru->pPSCStru = (DSU_PSC_STRU *)MyNew(sizeof(DSU_PSC_STRU) * dsuDataLenStru->wPSCLen);

						/*动态申请空间失败*/
						if (NULL == dsuStaticHeadStru->pPSCStru)
						{
							rtnValue = 0;
							funErrCode = DSU_FUNC_CODE4;
						}
						else
						{
							/*内存先设为无效*/
							memset(dsuStaticHeadStru->pPSCStru, 0xFF, sizeof(DSU_PSC_STRU) * dsuDataLenStru->wPSCLen);
						}
					}
				}
				else/*内存空间是由外部产品来分配的*/
				{
					/*检测电子地图对应数据项数组的个数是否超出内存分配范围*/
					dquGetEmapDataStruArrayLen(EMAP_PSC_ID, &wStruDataLen);

					if (((0 != wStruDataLen) && (dsuDataLenStru->wPSCLen >= wStruDataLen)) || (0 == wStruDataLen))
					{
						rtnValue = 0;
						funErrCode = DSU_FUNC_CODE5;
					}
					else
					{
						/*检测电子地图中对应索引数组的个数是否超出内存分配范围*/
						dquGetEmapDataIndexArrayLen(EMAP_PSC_ID, &wDataIndexLen);
						if (((0!=wDataIndexLen) && (dsuEmapIndexStru->PSCINDEXNUM >= wDataIndexLen)) || (0 == wDataIndexLen))
						{
							rtnValue = 0;
							funErrCode = DSU_FUNC_CODE6;
						}
						else
						{
							/*nothing to do*/
			...
			{
				/*解析数据到 自动折返 结构体数组 */
				for (j = g_struInitTaskTab[g_wCurInitStruProg].wCurInitPos; j < dsuDataLenStru->wPSCLen; j++)
				{
					pCurrentStru = dsuStaticHeadStru->pPSCStru + j;

					pData = pDataAddr + (rowDataLen * j);

					/*填充数据结构体*/
					PSC_STRU_init(pData, pCurrentStru);

					/*完成对应填充索引*/
					dsuEmapIndexStru->dsuPSCIndex[pCurrentStru->wId] = j;
				...
				rtnValue = (j == dsuDataLenStru->wPSCLen) ? (1) : (rtnValue);
			}
			else
			{
				/*nothing to do*/
	...
	else
	{
		/*空指针,直接返回失败*/
		rtnValue = 0;
		funErrCode = DSU_FUNC_CODE1;
	}

	/*故障码信息不为成功,打印*/
	if (DSU_FUNC_SUCESS != funErrCode)
	{
		printf("initPSC Err:0x%x",funErrCode);
	} 
	else
	{
		/*nothing to do*/
