...
/********如下2个字段用于完成站台表中动态加载站台对应触发区段*******/
#define STATION_INCLUDE_LOGICSGMT_NUM        ((UINT16)(100))          /*站台包含的最大触发逻辑区段数量 互联互通数据结构V18.0.0  add by sds 2020-02-11*/
#define STATION_NO_LOGICSGMT_SIZE            ((UINT16)(71))          /*站台表中排除触发区段ID后，其它字段的字节总数 互联互通数据结构V18.0.0  add by sds 2020-02-11*/
...
#define RELATE_ZC_AREA_INIT_NUM     25   /*2020.02.11 add by sds，根据ZC应用需求，增加ZC分界点最大共管区域数组个数,本宏用于数组大小定义,实际初始化数据时使用RELATE_ZC_AREA_NUM
										 **背景：ZC在新机场和武汉5号线等项目中，使用了该宏对应的结构体进行数据存储,目前数组大小无法满足实际使用，此处扩大了数组大小，实际初始化内容时依然使用数据中的实际大小进行初始化
										 */

/*单个ZC共管区域中包含的道岔数量*/
#define ZC_DIVID_SWITCH_NUM     8   /*2020.02.11 add by sds，根据ZC应用需求增加*/
#define ZC_DIVID_SWITCH_INIT_NUM     20   /*2020.02.11 add by sds，根据ZC应用需求，增加ZC共管区域内道岔数量,本宏用于数组大小定义,实际初始化数据时使用ZC_DIVID_SWITCH_NUM
										 **背景：ZC在新机场和武汉5号线等项目中，使用了该宏对应的结构体进行数据存储,目前数组大小无法满足实际使用，此处扩大了数组大小，实际初始化内容时依然使用数据中的实际大小进行初始化
										 */
...
typedef struct DSU_Data_Len
{
	...
	UINT16 wAdjtStationLen;	        /*临线停车区域表长度，依据互联互通数据结构V18.0.0添加，add by sds 20200211*/
	UINT16 wPSCLen;	                /*PSC表长度，依据互联互通数据结构V20.0.0添加，add by sds 2020-8-18*/
}DSU_DATA_LEN_STRU;
...
typedef struct DSU_StationStruct
{
	...
	UINT16  wTimeOfAdvanceRoute;            /*"提前触发发车进路时间，依据互联互通数据结构V18.1.0添加，add by sds 20200522*/
...
typedef struct DSU_ZCComZone
{
	...
	UINT16  wPointId[ZC_DIVID_SWITCH_INIT_NUM];                          /*共管区域内包含的道岔编号*/                 
	UINT16  wPointStatus[ZC_DIVID_SWITCH_INIT_NUM];                      /*共管区域内包含的道岔状态*/
...
typedef struct DSU_ZCDividPointStruct				
{
	...
	UINT16  wRelateZCAreaId[RELATE_ZC_AREA_INIT_NUM];  /*分界点相关的共管区域编号*/
...
/*临线处停车区域内部编号，依据互联互通数据结构V18.0.0添加，add by sds 20200211*/
typedef struct DSU_AdjtStationStruct
{
	UINT16	wId;						/*索引编号*/
	UINT16	wLineId;					/*所属线路编号*/
	UINT32	wLogicStaId;				/*所属逻辑站台编号*/
	CHAR	destID[4];					/*目的地编号*/
	CHAR	trackID[16];				/*轨道名称*/
	CHAR	stationID[20];				/*所属车站名称*/
	UINT16	adjtLineStaId;				/*联络线处停车区域内部编号*/

}DSU_AdjtStation_STRU;

/*PSC表结构*/
typedef struct DSU_PSCStruct
{
	UINT16	wId;						/*索引编号*/
	UINT16	wDevID;					    /*PSC的ID*/
	UINT16	wContainPSDID[4];			/*包含的PSD ID信息*/
	UINT16	wManagerCIID;			    /*所属CI ID信息*/
}DSU_PSC_STRU;
...
typedef struct DSU_PlatFormStruct
{
	...
	UINT16 wLogicSgmtId[STATION_INCLUDE_LOGICSGMT_NUM];						/*站台对应触发逻辑区段ID*/
...
typedef struct DSU_StaticHead/*存放静态数据各个数据结构的头指针*/
{
	 ...
	 DSU_AdjtStation_STRU *pAdjtStationStru;/*临线停车区域信息结构体指针，依据互联互通数据结构V18.0.0添加，add by sds 20200211*/
	 DSU_PSC_STRU *pPSCStru;                /*PSC信息结构体指针*/
...
typedef struct DSU_EmapIndexStruct
{
	...
	UINT16 ADJTSTATIONINDEXNUM;	    /*临线停车索引数组大小，依据互联互通数据结构V18.0.0添加，add by sds 20200211*/
	UINT16 *dsuAdjtStationIndex;	/*临线停车索引数组*/

	UINT16 PSCINDEXNUM;	            /*PSC索引数组大小*/
	UINT16 *dsuPSCIndex;	        /*PSC索引数组*/
