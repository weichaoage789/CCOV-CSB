...
/**************************************************************************
* 功能描述:		Link数据结构体初始化(互联互通)
* 输入参数:		UINT8 *pDataAdd  单条link数据行首地址
* 输入输出参数:	无
* 输出参数:		DSU_LINK_STRU *pLinkStrc 解析后的link数据信息
* 全局变量:		无。
* 返回值:		无。
* 修改记录:
* 1、增加link所属PSC区域编号信息初始化(互联互通数据结构V20.0.0) m by sds 2020-8-18
**************************************************************************/
static void LINK_STRU_init_HLT(UINT8 *pDataAdd, DSU_LINK_STRU *pLinkStrc)
{
	...
	if((NULL != pDataAdd) && (NULL != pLinkStrc))
	{
		...
		pLinkStrc->wManagePSCID = ShortFromChar(pData);  /*所属PSC区域编号*/
		pData += 2;
...
static void STATION_STRU_init_HLT(UINT8 * pDataAdd,DSU_STATION_STRU * pStationStru)
{
	...
	pStationStru->wTimeOfAdvanceRoute = ShortFromChar(pData); /*提前触发发车进路时间*/
	pData += 2;
...
参数说明：[in ] pDataAdd       数据地址（二进制文件）
		  ...
		  [out] realLoadRowLen 实际加载的停车点行数信息
		  [in ] stopPointAttr  过滤使用的停车点属性信息
返回值：0=失败  1=成功
备注：结构体需增加项
	UINT16  wMinStopTime;					最小停站时间
	UINT16  wMaxStopTime;					最大停站时间
	UINT16  wStopTime;						默认停站时间
修改记录：m by sds 2019-10-22 依据VOBC主线V15.0需求，完成停车点数据过滤，只保留满足相关输入停车点属性的数据
***************************/
UINT8 STOPPOINT_STRU_init(UINT8 * pDataAdd,DSU_STOPPOINT_STRU * pStoppointStru,UINT32 *realLoadRowLen,UINT32 stopPointAttr)
{
	UINT8 rtnValue = 0;
	UINT8 * pData = pDataAdd;
	...
	/*互联项目中 依据输入的停车点属性进行数据过滤，只保留满足条件的数据*/
	if (stopPointAttr == (stopPointAttr & pStoppointStru->wAttribute))
	{
	...
	{
		/*将已经处理的数据恢复为默认值*/
		pStoppointStru->wId = DSU_NULL_16;

		pStoppointStru->wAttribute = DSU_NULL_32;
	}
...
void RUN_LEVEL_STRU_init(UINT8 *pDataAdd,DSU_RUN_LEVEL_STRU *pRunLevelStru)
{
	...
	if ((DQU_CBTC_FAO == cbtcVersionType) || (DQU_CBTC_HLT == cbtcVersionType) ||(DQU_CBTC_DG == cbtcVersionType) )
	{
...
void ZC_COM_ZONE_STRU_init(UINT8 * pDataAdd,DSU_ZC_COM_ZONE_STRU * pZCComZoneStru)
{
   ...
   pZCComZoneStru->dwTmnlLinkOfst = LongFromChar(pData);
	...
	for(i=0;i<ZC_DIVID_SWITCH_NUM;i++)
	{
		/*共管区域内包含的道岔编号*/ 
...
输出参数：pPlatFormStru:输入内存地址对应的1行数据
返回值:无
**********************************************/
static void DSU_PLAT_FORM_STRU_init_HLT(UINT8 * pDataAdd, DSU_PLAT_FORM_STRU * pPlatFormStru,UINT16 logSgmtIDSumCount)
{
	UINT8 * pData = pDataAdd;/*局部临时指针*/
	UINT16 i = 0;/*用于函数内执行循环的变量*/
...
/*访问接口
**修改记录：
**1.根据互联互通数据结构V11.0.0需求修改，modify by lmy 20180208
**2.根据互联互通数据结构V18.0.0需求修改，m by sds 20200211
*/
void DSU_PLAT_FORM_STRU_init(UINT8 *pDataAdd, DSU_PLAT_FORM_STRU *pPlatFormStru,UINT16 logSgmtIDSumCount)
{
	UINT8 cbtcVersionType=0;
	cbtcVersionType=GetCbtcSysType();
	...
	{
		DSU_PLAT_FORM_STRU_init_HLT(pDataAdd, pPlatFormStru,logSgmtIDSumCount);
	}
	else
	{
       /*不可达分支*/
...
/**********************************************************
函数功能：临线停车区域表信息数据结构体初始化函数
参数说明：[in] pDataAdd 数据地址（二进制文件）
          [out]pAdjtStationStru被初始化的结构体指针
返回值：无
备注：依据互联互通数据结构V18.0.0添加，add by sds 20200211
***********************************************************/
void ADJTSTATION_STRU_init(UINT8 *pDataAdd, DSU_AdjtStation_STRU *pAdjtStationStru)
{
	UINT8 *pData = pDataAdd;
	UINT16 i=0;
	
	/*编号*/
	pAdjtStationStru->wId = ShortFromChar(pData);
	pData += 2;

	/*所属线路编号*/
	pAdjtStationStru->wLineId = ShortFromChar(pData);
	pData += 2;

	/*所属逻辑站台编号*/
	pAdjtStationStru ->wLogicStaId = LongFromChar(pData);
	pData += 4;

	for(i = 0; i < 4; i++)
	{
		pAdjtStationStru->destID[i] = *pData;
		pData++;
	}
	
	for(i = 0; i < 16; i++)
	{
		pAdjtStationStru->trackID[i] = *pData;
		pData++;
	}
	
	for(i = 0; i < 20; i++)
	{
		pAdjtStationStru->stationID[i] = *pData;
		pData++;
	}

	pAdjtStationStru ->adjtLineStaId = ShortFromChar(pData);
	pData += 2;
	
}

/**************************************************************************
* 功能描述:		PSC表信息数据结构体初始化函数
* 输入参数:		UINT8 *pDataAdd 数据地址（二进制文件）
* 输入输出参数:	无
* 输出参数:		DSU_PSC_STRU *pPSCStru 被初始化的结构体指针
* 全局变量:		无
* 返回值:		无
* 修改记录:
* 1、增加PSC表结构体初始化 m by sds 2020-8-18
**************************************************************************/
void PSC_STRU_init(UINT8 *pDataAdd, DSU_PSC_STRU *pPSCStru)
{
	UINT8 *pData = NULL;
	UINT16 loopIndex = 0;

	/*空指针防护*/
	if ((NULL != pDataAdd) && (NULL != pPSCStru))
	{
		pData = pDataAdd;

		/*索引编号*/
		pPSCStru->wId = ShortFromChar(pData);
		pData += 2;

		/*PSC的ID*/
		pPSCStru->wDevID = ShortFromChar(pData);
		pData += 2;

		/*包含的PSD ID*/
		for(loopIndex = 0; loopIndex < 4; loopIndex++)
		{
			pPSCStru->wContainPSDID[loopIndex] = ShortFromChar(pData);
			pData += 2;
		}

		/*所属CI的 ID*/
		pPSCStru ->wManagerCIID = ShortFromChar(pData);
		pData += 2;
	} 
	else
	{
