...
static UINT8 initConfigCbtcInfoTaskTab(void)
{
	...
	if (DQU_CBTC_CPK == cbtcVersionType)/*合库功能配置*/
	{
		...
		g_struInitTaskTab[6].bIsEffective=DQU_QFUN_FASLE;/*合库默认不执行，车辆段配置*/
		rtnValue = 1;
	}	
...
UINT8 dfsInitSubSystemCommonConfigData_HLT(UINT8 *pDataAddr, UINT32 dataSize, UINT16 rowCount)
{
	...
	if ((NULL != pDataAddr) && (1 == rowCount))
	{
		...
		/*互联互通厂家代码中"通号"所属编号”类型,依据互联互通数据结构V18.0.0添加，add by sds 2019-02-12*/
		g_pCbtcConfigDataStru->cbtcConfigData.FactoryCodeOfCRSC = ShortFromChar(pDataAddr);
		pDataAddr += 2;

		/*互联互通厂家代码中"交控"所属编号”类型,依据互联互通数据结构V18.0.0添加，add by sds 2019-02-12*/
		g_pCbtcConfigDataStru->cbtcConfigData.FactoryCodeOfTCT = ShortFromChar(pDataAddr);
		pDataAddr += 2;

		/*互联互通厂家代码中"铁科"所属编号”类型,依据互联互通数据结构V18.0.0添加，add by sds 2019-02-12*/
		g_pCbtcConfigDataStru->cbtcConfigData.FactoryCodeOfTK = ShortFromChar(pDataAddr);
		pDataAddr += 2;

		/*互联互通厂家代码中"众合"所属编号”类型,依据互联互通数据结构V18.0.0添加，add by sds 2019-02-12*/
		g_pCbtcConfigDataStru->cbtcConfigData.FactoryCodeOfZH = ShortFromChar(pDataAddr);
		pDataAddr += 2;

		/*PSC-CI通信故障判断时间(ms)  互联互通数据结构V20.0.0*/
		g_pCbtcConfigDataStru->cbtcConfigData.CommFaultTimeOfPSC_CI = LongFromChar(pDataAddr);
		pDataAddr += 4;

		/*PSC-VOBC通信故障判断时间(ms)  互联互通数据结构V20.0.0*/
		g_pCbtcConfigDataStru->cbtcConfigData.CommFaultTimeOfPSC_VOBC = LongFromChar(pDataAddr);
		pDataAddr += 4;

		/*PSC-IM通信故障判断时间(ms)  互联互通数据结构V20.0.0*/
		g_pCbtcConfigDataStru->cbtcConfigData.CommFaultTimeOfPSC_IM = LongFromChar(pDataAddr);
		pDataAddr += 4;

		/*VOBC-PSC接口协议类型  互联互通数据结构V20.0.0*/
		g_pCbtcConfigDataStru->cbtcConfigData.CommProtocolTypeOfVOBC_PSC = *pDataAddr;
		pDataAddr += 1;

		/*VOBC-PSC接口协议版本号  互联互通数据结构V20.0.0*/
		g_pCbtcConfigDataStru->cbtcConfigData.CommProtocolVerOfVOBC_PSC = *pDataAddr;
		pDataAddr += 1;

		/*VOBC与MSS通信时使用接口协议类型  互联互通数据结构V20.0.0*/
		g_pCbtcConfigDataStru->cbtcConfigData.CommProtocolTypeOfVOBC_MSS = *pDataAddr;
		pDataAddr += 1;

		/*VOBC与CI通信时使用接口协议类型  互联互通数据结构V20.0.0*/
		g_pCbtcConfigDataStru->cbtcConfigData.CommProtocolTypeOfVOBC_CI = *pDataAddr;
		pDataAddr += 1;

		/*VOBC与ZC通信时使用接口协议类型  互联互通数据结构V20.0.0*/
		g_pCbtcConfigDataStru->cbtcConfigData.CommProtocolTypeOfVOBC_ZC = *pDataAddr;
		pDataAddr += 1;

		/*VOBC与ATS通信时使用接口协议类型  互联互通数据结构V20.0.0*/
		g_pCbtcConfigDataStru->cbtcConfigData.CommProtocolTypeOfVOBC_ATS = *pDataAddr;
		pDataAddr += 1;
