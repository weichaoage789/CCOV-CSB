...
UINT8 dataReadNoCopy(CHAR * dataName)
{
	...
	FILE * pfile = NULL;
	DH	dh = {0};
	UINT8 *  pDataAddr = (UINT8 * )dataName;

	/*判定是否为第一次加载,防止重复操作*/
	if((0 == isDataRead) && (NULL != pDataAddr))
	{
		/*前4个字节为文件长度，跳过*/
		g_pDataBuf = (pDataAddr + 4);
		pData = g_pDataBuf;
		...
		/*文件长度错误*/
		if((4 > (dh.dataLength)) || (DFS_MAX_FILE_LEN <= (dh.dataLength)))
		{
			fileErrNo |= ERR_FILE_LENTH;
			rtnValue = 0;
		}
		else
		{
			...
			/*提取CRC信息*/
			tmpValue =  LongFromChar(g_pDataBuf);
				
			/*CRC校验*/
			if(CRCresult != tmpValue)
			{
				fileErrNo |= ERR_CRC_CHECK;
				rtnValue = 0;
			}
			else
			{
				...
				if (NULL == g_pDFsDH)
				{
					...
					{
						/*FS文件数据头结构信息置为0x00*/
						memset(g_pDFsDH,0x00,sizeof(dfsDataHead));
