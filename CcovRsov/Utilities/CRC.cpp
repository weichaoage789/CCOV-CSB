...
void Init(CRC_CFG *cfg, unsigned char order, unsigned int poly, unsigned int crcinit, unsigned int crcxor, unsigned char refin, unsigned char refout)
{
	...
	cfg->crcinit = crcinit;
	cfg->crcxor = crcxor;
	cfg->crcmask = ((((unsigned int)1 << (order - 1)) - 1) << 1) | 1;
	cfg->crchighbit = (unsigned int)1 << (order - 1);

	/*generate lookup table*/
	GenerateCrcTable(cfg);
}
...
unsigned int Crc(const unsigned char *data, const unsigned int len, CRC_CFG *cfg)
{
	...
	if (nullptr != data && 0 < len)
	{
		...
		if (cfg->refin == 0)
		{
			for (int i = 0; i < len; i++)
			{
				crc = (crc << 8) ^ cfg->crctab[((crc >> (cfg->order - 8)) & 0xff) ^ data[i]];
			}
		}
		else
		{
			for (int i = 0; i < len; i++)
			{
				crc = (crc >> 8) ^ cfg->crctab[(crc & 0xff) ^ data[i]];
			}
		}

		if ((cfg->refout ^ cfg->refin) != 0)
		{
			Reflect(&crc, cfg->order);
		}
...
unsigned short Crc16(const unsigned char *data, const unsigned int len)
{
	...
	const static unsigned short CrcTable16[256] = {
	...
	if (nullptr != data && 0 < len)
	{
		while (i < len)
		{
			rt_crc = (rt_crc << 8) ^ CrcTable16[(rt_crc >> 8) ^ data[i]];
			i++;
		}
	}
...
{
	unsigned int i = 0;
	unsigned int splite_len = 0;
	unsigned int cacl_cnt = 0;

	/** @initial the crc result value to 0 */
	*crcrslt = 0;

	/*生成多项式：0x4c11db7L;  */
	const static unsigned int CrcTable[256] = {
	...
	if (nullptr != data && 0 != len)
	{
		if (0 != caclnum)
		{
			splite_len = len / caclnum;

			if (cacl_cnt < caclnum)
			{
				while (i < (splite_len + (cacl_cnt * splite_len)))
				{
					*crcrslt = (*crcrslt << 8) ^ CrcTable[(*crcrslt >> 24) ^ data[i]];
					i++;
				}
				/*累加校验的周期数*/
				cacl_cnt++;
...
unsigned int Crc32(const unsigned char *data, const unsigned int len)
{
	...
	while (i < len)
	{
		rtCrc = (rtCrc << 8) ^ CrcTable[(rtCrc >> 24) ^ data[i]];
		i++;
	}
	return rtCrc;
}

unsigned char BigFileCrc32(const unsigned char *data, unsigned int len, unsigned char CrcNum, unsigned int *CrcCode)
{
	static   unsigned int i = 0;
	static unsigned int SplitLength = 0;
	static unsigned char CalculateNum = 0;
	unsigned char CrcFlag = 0xff;
	unsigned char M = 0;
	...
	if ((0 != len) && (0 != CrcNum) && (0xff == CrcFlag))
	{
		SplitLength = len / CrcNum;
		...
		if (CalculateNum < CrcNum)
		{
			...
			{
				*CrcCode = (*CrcCode << 8) ^ CrcTable[(*CrcCode >> 24) ^ data[i]];
				i++;
			}
			/*累加校验的周期数*/
